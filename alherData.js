export const alherData = {
  nombre: "Grupo Educativo Alher",
  años: 49,
  valores: ["Integridad", "Respeto", "Responsabilidad", "Excelencia", "Solidaridad"],
  fundador: "Prof. Alfredo Herrera Meneses",
  mision:
    "Formar seres humanos integrales y empoderados, capaces de enfrentar desafíos personales y profesionales con confianza y fortalezas académicas, mediante el compromiso de sus colaboradores y la generación de un entorno propicio, para contribuir a una sociedad equitativa y sustentable.",
  vision:
    "Convertirse en el Grupo Educativo líder en excelencia académica, proporcionando herramientas para formar líderes con sólidos valores, que aprecien, practiquen y fomenten la cultura, el deporte y la innovación tecnológica.",
  distintivos: [
    "Sistema UNOi (de Preescolar a Secundaria)",
    "Apple Teacher",
    "Google Workspace for Education",
    "Microsoft 365",
    "Certificaciones Cambridge",
    "Certificaciones Microsoft Office (TestingProgram)",
    "Biblioteca escolar",
    "Artes",
    "Juegos Matemáticos",
    "Comprensión lectora",
    "Cultura Maker",
    "Pedagogía Crítica",
    "Pedagogía Líquida",
    "Pedagogía Maker",
    "Apoya Alher (Apoyo psicopedagógico)",
    "Enfermería escolar",
    "Mindfulness",
    "Progrentis",
    "Inglés intensivo"
  ],
  niveles: {
    estancia: {
      nombre: "Estancia Infantil",
      descripcion:
        "En la Estancia Infantil Alher, los pequeños desarrollan sus primeras habilidades sociales, motrices y cognitivas en un entorno seguro, cálido y estimulante, con atención personalizada y espacios diseñados especialmente para ellos.",
    },
    maternal: {
      nombre: "Maternal",
      descripcion:
        "En Maternal Alher se fomenta el desarrollo del lenguaje, la socialización y las habilidades motrices mediante juegos, canciones, actividades artísticas y un enfoque de descubrimiento y aprendizaje significativo.",
    },
    preescolar: {
      nombre: "Preescolar",
      descripcion:
        "El nivel Preescolar Alher impulsa la curiosidad, la autonomía y el pensamiento lógico mediante programas como Progrentis y Mindfulness, incorporando inglés, artes e innovación tecnológica con el sistema UNOi.",
    },
    primaria: {
      nombre: "Primaria",
      descripcion:
        "En Primaria Alher los alumnos aprenden con un modelo educativo integral que promueve el pensamiento crítico, la creatividad y la comprensión lectora, con aulas equipadas con tecnología Apple y el sistema UNOi.",
    },
    secundaria: {
      nombre: "Secundaria",
      descripcion:
        "La Secundaria Alher refuerza la autonomía y las habilidades académicas con el sistema UNOi, programas de comprensión lectora, Cultura Maker y certificaciones internacionales, preparando a los estudiantes para el bachillerato.",
    },
    bachillerato: {
      nombre: "CCH Alher (Incorporado a la UNAM)",
      descripcion:
        "El Colegio de Ciencias y Humanidades Alher ofrece una educación media superior con incorporación a la UNAM, complementada con distintivos Apple Teacher, Google Workspace y Microsoft 365, enfocada en el desarrollo académico, digital y humano.",
    },
  },
  planteles: {
    aragon: {
      nombre: "Plantel Aragón",
      direccion:
        "Av. Valle del Don #5, Valle de Aragón 3a Sección, Ecatepec, Estado de México.",
      telefono: "+52 56 1538 1099",
      niveles: [
        "Estancia Infantil",
        "Maternal",
        "Preescolar",
        "Primaria",
        "Secundaria",
        "CCH Alher (Incorporado a la UNAM)",
      ],
    },
    santiago: {
      nombre: "Plantel Santiago",
      direccion:
        "Av. Valle de Santiago, Valle de Aragón 1a Sección, Nezahualcóyotl, Estado de México.",
      telefono: "+52 56 1538 1099",
      niveles: ["Maternal", "Preescolar", "Primaria"],
    },
    acueducto: {
      nombre: "Plantel Acueducto",
      direccion:
        "Av. Acueducto #142, Col. Jorge Jiménez Cantú, Ecatepec, Estado de México.",
      telefono: "+52 55 5116 0694",
      niveles: ["Preescolar", "Primaria", "Secundaria", "Bachillerato (SEP)"],
    },
  },
  promociones: {
    alherDays: {
      nombre: "AlherDays",
      descripcion:
        "Durante AlherDays, puedes obtener descuentos especiales en pagos de servicios de hasta el 20% y en colegiaturas de hasta el 25%, dependiendo del nivel educativo a inscribir.",
      vigencia: "Del 13 al 19 de noviembre.",
    },
  },
};

export default alherData;
